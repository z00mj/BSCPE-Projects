...
if (isset($_GET['collection_id'])) {
    $_SESSION['redirect_url'] = 'collection.php?collection_id=' . $_GET['collection_id'];
}
...
if (!isset($_GET['collection_id'])) {
    // Try to get the collection_id from the session if available
    if (isset($_SESSION['last_collection_id'])) {
        $collectionId = intval($_SESSION['last_collection_id']);
        // Store in session for future use
        $_SESSION['last_collection_id'] = $collectionId;
    } else {
        // If no collection_id is found, redirect to marketplace
        header("Location: marketplace.php");
        exit();
    }
} else {
    // Store the collection_id in session for future use
...
try {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        try {
            ...
            // Check if any NFT from this collection is in user's favorites
            $favStmt = $pdo->prepare("SELECT f.favorite_id 
                                    FROM luck_wallet_user_favorites f 
                                    JOIN luck_wallet_nfts n ON f.nft_id = n.nft_id 
                                    WHERE f.user_id = ? AND n.collection_id = ? 
                                    LIMIT 1");
            $favStmt->execute([$userId, $collectionId]);
    ...
    $collectionStmt = $pdo->prepare("
        SELECT 
            ...
            c.name,
            c.description,
            c.logo_url as banner_image_url,
            c.logo_url,
            c.creator_user_id as owner_id,
            (SELECT COUNT(*) FROM luck_wallet_nfts WHERE collection_id = c.collection_id) as nft_count,
            (SELECT COUNT(DISTINCT current_owner_user_id) FROM luck_wallet_nfts WHERE collection_id = c.collection_id) as owner_count,
            (SELECT MIN(price_luck) FROM luck_wallet_nfts WHERE collection_id = c.collection_id AND listed_for_sale = 1) as floor_price,
            (SELECT SUM(price_luck) FROM luck_wallet_nfts WHERE collection_id = c.collection_id) as total_volume
    ...
    $nftsStmt->execute([$isLoggedIn ? $userId : 0, $collectionId]);
    ...
    header("Location: marketplace.php?error=collection_not_found");
    exit();
}
...
<head>
    ...
    <title><?php echo htmlspecialchars($collection['name']); ?> - LUCK Marketplace</title>
        ...
        <main class="main-content">
            <section class="hero-section">
                <div class="hero-image">
                    <?php 
                        $logoUrl = !empty($collection['logo_url']) ? 
                            (strpos($collection['logo_url'], 'http') === 0 ? $collection['logo_url'] : '../' . ltrim($collection['logo_url'], '/')) : 
                            'default-logo.png';
                    ?>
                    <img src="<?php echo htmlspecialchars($logoUrl); ?>" alt="<?php echo htmlspecialchars($collection['name']); ?>" 
                ...
                <div class="hero-content">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; width: 100%;">
                        <h1><?php echo htmlspecialchars($collection['name']); ?> <span class="collection-badge">Collection</span></h1>
                    </div>
                    <p class="hero-description"><?php echo htmlspecialchars($collection['description']); ?></p>
                    <div class="hero-stats">
                        <div class="hero-stat-item">
                            <div class="value"><?php echo number_format($collection['nft_count']); ?></div>
                            <div class="label">Items</div>
                        </div>
                        <div class="hero-stat-item">
                            <div class="value"><?php echo number_format($collection['floor_price'] ?? 0, 2); ?> LUCK</div>
                            <div class="label">Floor Price</div>
                        </div>
                        <div class="hero-stat-item">
                            <div class="value"><?php echo number_format($collection['total_volume'] ?? 0, 2); ?> LUCK</div>
        ...
        function displaySearchResults(results) {
            ...
            results.forEach(result => {
                ...
                item.addEventListener('click', () => {
                    console.log('Clicked on collection:', result.collection_id);
