...
<body>
        ...
        <nav>
            ...
            <div class="user-profile-bubble" id="userProfileBubble">
                ...
                <div class="dropdown-content">
                    <a href="/Luck%20Wallet/marketplace/profile.php" class="dropdown-item">
                        <i class="fas fa-user" style="margin-right: 8px;"></i>View Profile
                    </a>
                    <a href="/Luck%20Wallet/auth/logout.php" class="dropdown-item">
    ...
    <main>
        <div class="dashboard-top-row">
            ...
            <div class="tasks-section" style="overflow-y: auto; -ms-overflow-style: none; scrollbar-width: none;">
            <style>.tasks-section::-webkit-scrollbar { display: none; }</style>
                <h2>EARN $LUCK</h2>
                <div class="task-item">
                ...
                <div class="task-item">
                    <p>Mine $LUCK</p>
                    <button class="claim-button" id="mineLuckButton" data-task-type="mine">Go</button>
                </div>
                <div class="task-item">
    ...
    <script>
            ...
            try {
                ...
                if (nfts.length > 0) {
                    collectionContainer.innerHTML = `
                        ...
                        ${nfts.map(nft => `
                            <div class="nft-card" data-nft-id="${nft.id}" data-collection-id="${nft.collection_id || ''}" onclick="handleNftClick(event, ${nft.id}, ${nft.collection_id || 'null'})">
        ...
        // --- Mine $LUCK Button Logic ---
        const mineLuckButton = document.getElementById('mineLuckButton');
        if (mineLuckButton) {
            mineLuckButton.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
    ...
    <script>
        ...
        function handleNftClick(event, nftId, collectionId) {
            // Prevent default link behavior
            event.preventDefault();
            event.stopPropagation();
            
            // If the click was on a link or button inside the card, don't navigate
            if (event.target.tagName === 'A' || event.target.tagName === 'BUTTON' || 
                event.target.closest('a') || event.target.closest('button')) {
                return;
            }
            
            // If the NFT has a collection, go to the collection page
            if (collectionId) {
                window.location.href = `marketplace/collection.php?collection_id=${collectionId}`;
            } else {
                // Fallback to NFT details if no collection
                window.location.href = `nft_details.php?id=${nftId}`;
            }
        }
    </script>
</html>